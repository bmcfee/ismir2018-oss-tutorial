...
When you say `pip install <some-package-name>` or `conda install <some-package-name>`, you're installing a
...
The second line imports any variables, classes, and functions defined in the `freq.py` module.
After saying `import toymir`, a user can access functions as `toymir.hz_to_period()` (for
example).
...
Next, type `toymir.<TAB>` and you should see a table of the contents of the `toymir` package, which you can navigate by arrow keys.
Select `hz_to_period` and press enter: this should leave `toymir.hz_to_period` on your input prompt.

Now, add a `?` to the end of the line, so it reads `toymir.hz_to_period?` and press `<ENTER>`.
This will bring up the documentation for the `hz_to_period` function, which you can quit by hitting `q`.
If you add two `??` instead of one, it will bring up both the documentation and the source code (if available).
...
You may have noticed that the import lines in `__init__.py` above look funny.  You might be used to seeing
...
Instead of `toymir.hz_to_period`, the function should now be located at `toymir.freq.hz_to_period`.
...
Ready to keep going?  Onward to [Part 5][tutorial-part-5].
...
[tutorial-part-5]: https://bmcfee.github.io/ismir2018-oss-tutorial/tutorial/2018/08/12/part-5.html
